...
var NodePrivateZstdFrameDecoder = class NodePrivateZstdFrameDecoder {
	...
	*/
	static create() {
		if (typeof nodeZlib.createZstdDecompress !== "function") return void 0;
	...
	decodeFrame(input) {
		...
		for (;;) {
			...
			if (produced > 0) {
				...
				if (outputBytes > constants.MAX_LENGTH) throw new Error(`Zstandard frame output exceeds ${constants.MAX_LENGTH} bytes`);
			}
			if (outputAfter !== 0) {
...
//#endregion
//#region lib/types/zstd-backend.js
/**
...
const nativeZstdAvailable = typeof nativeZlib.zstdCompress === "function";
const CHECKSUM_OPTIONS = { params: { [nativeZlib.constants?.ZSTD_c_checksumFlag ?? 0]: 1 } };
const PARTIAL_FLUSH_OPTIONS = { finishFlush: nativeZlib.constants?.ZSTD_e_flush ?? 0 };
let wasm;
if (!nativeZstdAvailable) wasm = await (await import("@hpcc-js/wasm-zstd")).Zstd.load();
/** Compress one independently decodable, checksummed Zstandard frame. */
async function compressFrameBackend(input) {
	if (nativeZstdAvailable) return await new Promise((resolve, reject) => {
...
async function decompressFrameBackend(input) {
	if (nativeZstdAvailable) return await new Promise((resolve, reject) => {
		nativeZlib.zstdDecompress(input, void 0, (error, result) => {
			if (error) reject(error);
			else resolve(result);
		});
	});
...
*/
async function decompressPrefixBackend(input) {
	if (nativeZstdAvailable) return await new Promise((resolve, reject) => {
...
function decompressSyncBackend(input) {
	if (nativeZstdAvailable) return nativeZlib.zstdDecompressSync(input);
	return Buffer.from(wasm.decompress(input));
}
//#endregion
