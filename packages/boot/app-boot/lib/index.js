...
*/
const parseEnv = nodeUtil.parseEnv ?? parseEnvShim;
/**
...
function parseEnvValue(value, env, defined) {
	const v = value.trim();
	if (v.length >= 2 && v[0] === "'" && v.endsWith("'")) return v.slice(1, -1).replace(/\\'/g, "'");
	if (v.length >= 2 && v[0] === "\"" && v.endsWith("\"")) {
		let inner = v.slice(1, -1);
		inner = inner.replace(/\\(n|r|t|v|0)/g, (_, c) => ({
			n: "\n",
			r: "\r",
			t: "	",
			v: "\v",
			"0": "\0"
		})[c]).replace(/\\x([0-9a-fA-F]{2})/g, (_, h) => String.fromCharCode(parseInt(h, 16))).replace(/\\u(\{[0-9a-fA-F]+\}|[0-9a-fA-F]{4})/g, (_, h) => String.fromCodePoint(parseInt(h.replace(/[{}]/g, ""), 16))).replace(/\\(.)/g, (_, c) => c);
		return expandVars(inner, env, defined);
	}
	let un = value.trim();
	const hash = un.indexOf("#");
	if (hash !== -1) un = un.slice(0, hash);
	un = un.replace(/\\(.)/g, (_, c) => c);
	return expandVars(un, env, defined);
}
function expandVars(s, env, defined) {
	return s.replace(/\$\{(\w+)\}|\$(\w+)/g, (_, b, c) => {
		const name = b ?? c;
